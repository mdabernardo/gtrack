...
def sync_optimization_to_firestore(
    ...
    try:
        ...
        # Primary data doc
        doc_ref = db.document(
            'artifacts', project_id, 'public', 'data', 'optimizations', doc_id
        )
        doc_ref.set(payload, merge=True)

        # Index by day for quick lookup
        index_ref = db.document(
            'artifacts', project_id, 'public', 'index', 'optimizations_by_day', iso_date, str(route_id)
        )
        index_ref.set(payload, merge=True)
...
def sync_scheduling_assistance_to_firestore(
    ...
    try:
        ...
        # Primary data doc
        doc_ref = db.document(
            'artifacts', project_id, 'public', 'data', 'scheduling_assistance', doc_id
        )
        doc_ref.set(payload, merge=True)

        # Index by day for quick lookup
        index_ref = db.document(
            'artifacts', project_id, 'public', 'index', 'scheduling_by_day', iso_date, str(route_id)
        )
        index_ref.set(payload, merge=True)
